...
class Bot:
    ...
    def get_headers_with_jwt(username, password):
        request = requests.post('http://127.0.0.1:8000/api/token/', data={'username': username, 'password': password})
        jwt = request.json()
        headers = {
    ...
    def bot_from_config(cls, file_path):
        ...
        bot = cls(users_amount=users_amount, max_posts_amount=max_posts_amount, max_likes_amount=max_likes_amount)
